...
- 最终答案原则上必须能解析为题目要求的 `<result>...</result>` JSON 列表。评分时允许一种保守的格式恢复：若包装完全缺失，但最终回答中存在唯一、无冲突的独立代码块，且其中完整故障列表与某个标准答案选项精确一致，则按语义正确计分并标记 `recovered_fenced_exact_match`；正文提及、模糊片段、冲突代码块和近似匹配均不恢复。
