...
`data/simulation/` 保存 IP 故障分析仿真的原始提示词和训练 JSONL。资料包括：
...
仿真目录中的 ZIP 和 RAR 压缩包已移除，不再随仓库分发。
...
- 2026-07-27：删除 `data/simulation/` 下的 2 个 ZIP 和 2 个 RAR 压缩包，保留提示词与可直接读取的 JSONL 数据。
