"""Force and audit the fixed learning rate for one resumed 0809 stage."""
...
def _required_env(name: str) -> str:
    ...
    if not value:
        raise RuntimeError(f"{name} is required by the 0809 fixed-stage LR plugin")
    return value


STAGE = int(_required_env("QWEN36_0809_TRAIN_STAGE"))
TARGET_LR = float(_required_env("QWEN36_0809_TARGET_LR"))
AUDIT_PATH = Path(_required_env("QWEN36_0809_LR_AUDIT_PATH"))
PROCESS_RANK = int(os.environ.get("RANK", "0"))

if STAGE not in range(1, 6):
    raise RuntimeError(f"QWEN36_0809_TRAIN_STAGE must be 1..5, found {STAGE}")
if not math.isfinite(TARGET_LR) or TARGET_LR <= 0:
    raise RuntimeError(f"QWEN36_0809_TARGET_LR must be positive, found {TARGET_LR}")
...
class Qwen360809FixedStageLRCallback(TrainerCallback):
    @staticmethod
    ...
    def on_step_begin(self, args, state, control, optimizer=None, lr_scheduler=None, **kwargs):
        self._apply(optimizer, lr_scheduler)
...
callbacks_map["qwen36_0809_fixed_stage_lr"] = Qwen360809FixedStageLRCallback
