...
def main() -> None:
    ...
    formal_config = json.loads(
        (ROOT / "config" / "qwen36_0809_formal_training.json").read_text(
            encoding="utf-8"
        )
    )
    evaluation_policy = formal_config["evaluation_policy"]
    ...
    metrics = {
        ...
        "agent_evaluation_protocol": {
            **evaluation_policy,
            "manifest_matches_formal_config": manifest.get(
                "evaluation_protocol"
            )
            == evaluation_policy,
            "scoring_implementation_tracked": "final_answer_scorer"
            in manifest["reproducibility"]["tracked_files"],
            "launcher_implementation_tracked": "agent_validation_launcher"
            in manifest["reproducibility"]["tracked_files"],
        },
...
v7 针对 2026-08-11 20:02 深审将 action 门禁升级为完整结构门禁：从首个正 loss 前的真实工具回显重算同题所有严格事实 binding，不再信任 metadata。完整 binding 一律停止，未完整 binding 一律只监督已匹配事实、缺失条件、待验证假设和下一步只读动作。endpoint 也从“正证据顺带排除邻近标签”改为候选范围校准：正证据只确认当前标签/当前证据集合，未经候选专属反证的邻近标签保持未验证。Agent 判分现统一绑定仓库 canonical v3 final-answer-only scorer：过程错误只作诊断，q73–q86 inclusive-OR 继续由同一入口处理。训练/验证划分、每轮 1151 行、144 optimizer step、固定 epoch 3 / global step 432 均保持不变。
