...
AGENT_VALIDATION_CASE_IDS="${AGENT_VALIDATION_CASE_IDS:-}"
...
SERVED_MODEL_NAME="${SERVED_MODEL_NAME:-Qwen3.6-27B-trained}"
...
if [[ -z "${AGENT_VALIDATION_CASE_IDS}" ]]; then
  AGENT_VALIDATION_CASE_IDS="$(
    "${TRAIN_PYTHON}" - "${MANIFEST_PATH}" <<'PY'
import json
import sys
manifest = json.load(open(sys.argv[1], encoding="utf-8"))
case_ids = manifest.get("split", {}).get("validation_case_ids") or []
if not case_ids:
    raise SystemExit("manifest has no validation_case_ids")
