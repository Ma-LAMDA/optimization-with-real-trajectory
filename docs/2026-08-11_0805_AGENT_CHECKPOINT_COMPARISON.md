...
- 计分版本：`agent-final-answer.v4.2026-08-12-incomplete-result-exact-recovery`。
...
- v4 仅把全篇唯一、完整 fenced `<result>` JSON 字符串列表缺少 `</result>` 且精确命中可接受答案的输出恢复为正确；其他残缺或冲突输出仍判错。原 0805 epoch-1/2/3/4/5 的重算结果均未因此变化。
...
| 0805 epoch-3 | 432 | 35/60 | 58.33% | 完整，v4 重算 | 35/60（58.33%） |
