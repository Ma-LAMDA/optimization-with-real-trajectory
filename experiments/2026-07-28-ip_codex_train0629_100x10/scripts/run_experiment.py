...
def initialize_or_validate(base_url: str, safe_index: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    manifest = {
        "schema_version": "ip-codex-distillation-manifest.v1",
        "status": "running",
        "started_at": utc_now(),
        ...
        "generator_input_fields": ["question", "output_format", "localized original user prompt", "local API URL"],
    ...
    state = {
        "schema_version": "ip-codex-distillation-state.v1",
        "status": "running",
...
def execute_attempt(task: dict[str, Any]) -> dict[str, Any]:
    ...
    events_text = (attempt_dir / "events.jsonl").read_text(encoding="utf-8", errors="replace")
    failure_text = "\n".join([stderr_text, events_text, launch_error or ""])
...
def apply_outcome(state: dict[str, Any], task: dict[str, Any]) -> str | None:
    ...
    if outcome == "correct":
        ...
        sample["status"] = (
            ...
            if sample["consecutive_wrong"] >= MAX_CONSECUTIVE_WRONG
            else "pending"
        )
...
def write_final_report(manifest: dict[str, Any], state: dict[str, Any]) -> None:
    ...
    summary = {
        "schema_version": "ip-distill-final-summary.v1",
    ...
    lines.extend(["", "## 逐题状态", "", "| row | 原始 ID | accepted | 连续错误 | 总 attempt | 模型调用 | 基础设施失败 | 最终状态 |", "|---:|---:|---:|---:|---:|---:|---:|---|"])
    for sample in state["samples"]:
        lines.append(
            f"| {sample['row_index']} | {sample['original_id']} | {sample['accepted_count']} | {sample['consecutive_wrong']} | {sample['total_attempts']} | {sample['total_model_attempts']} | {sample['infrastructure_failures']} | {sample['status']} |"
        )
...
def main() -> int:
    ...
    try:
        ...
        elif state.get("status") == "stopped_by_infrastructure_blocker":
            log("checkpoint is stopped by an infrastructure blocker; preserving stopped state")
        else:
