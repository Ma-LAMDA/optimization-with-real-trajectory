...
def main() -> int:
    ...
    if all(metadata_present):
        ...
        source_template = str(prompts[0])
        prompt_source = "metadata.user_prompt"
    else:
        prompt_source = "data/simulation/IP user prompt.txt (repository-documented fallback; source rows have no metadata)"

    remote_url = "http://10.139.194.154:3080"
    remote_host = "10.139.194.154"
    if remote_url in source_template:
        ...
        localized = source_template.replace(remote_url, args.base_url)
        localized = localized.replace(remote_host, parsed_local.hostname)
        if parsed_local.port != 3080:
            localized = localized.replace("`3080` 端口", f"`{parsed_local.port}` 端口")
        replacement = {
            "operation": "service address adaptation only",
            "from": remote_url,
            "to": args.base_url,
            "host_occurrences_replaced": source_template.count(remote_host),
        }
    elif source_template.count(remote_host) == 2:
        replacement_host = args.base_url.split("//", 1)[-1].rsplit(":", 1)[0]
        localized = source_template.replace(remote_host, replacement_host)
        replacement = {"from": remote_host, "to": replacement_host}
    else:
        raise ParseError("source prompt does not contain the documented remote service address")
    if remote_host in localized:
        raise ParseError("remote service host remains after localization")
