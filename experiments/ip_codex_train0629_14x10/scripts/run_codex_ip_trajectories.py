...
DEFAULT_DATASET = EXPERIMENT_ROOT / "inputs" / "train_0629.jsonl"
DEFAULT_TEMPLATE = (
    EXPERIMENT_ROOT / "inputs" / "IP user prompt with saved configs skills.txt"
)
DEFAULT_OUTPUT_ROOT = EXPERIMENT_ROOT / "results" / "runs"
DEFAULT_CASE_IDS = (13, 14, 17, 18, 25, 26, 27, 28, 87, 88, 91, 92, 93, 94)
DEFAULT_REPEATS = 10
...
def main() -> int:
    ...
    if resumed_manifest is not None and resumed_run_dir is not None:
        ...
        if manifest.get("template_sha256") != digest(template_path):
            raise ValueError("Template hash differs from the resumed run")
        expected_slots = [
            (row["id"], repeat_index)
