...
def main() -> int:
    ...
    for sample in state['samples']:
        ...
        if (
            sample['status'] == 'abandoned_after_10_consecutive_wrong'
            and sample['consecutive_wrong'] < MAX_CONSECUTIVE_WRONG
        ):
            final_state_errors.append(f"row_{sample['row_index']}_consecutive_wrong_threshold")
        if (
            sample['status'] == 'abandoned_after_20_total_wrong'
            and sample['total_wrong'] < MAX_TOTAL_WRONG
        ):
            final_state_errors.append(f"row_{sample['row_index']}_total_wrong_threshold")
        if sample['total_wrong'] < sample['consecutive_wrong']:
            final_state_errors.append(f"row_{sample['row_index']}_wrong_count_order")
